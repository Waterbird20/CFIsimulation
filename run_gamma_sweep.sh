...
# Sweep over p values, gamma_pd values (fixed t_s), and ps_gamma values (fixed post-selection strength)
...
for p in 1.0 1.25 1.5 1.75 2.0 2.5 3.0; do
  for gamma_pd in 0.05 0.10 0.15 0.20 0.25 0.30 0.35 0.40 0.45 0.50 0.55 0.60 0.65 0.70 0.75 0.80 0.85 0.90; do
    for ps_gamma in 0.0 0.25 0.5 0.75; do
...
")
      ...
      tag="fixed_p${p}_g${gamma_pd}_ps${ps_gamma}"
...
save_to:         ${tag}
EOF
      echo "=== Running p=${p}, gamma_pd=${gamma_pd}, ps_gamma=${ps_gamma}, fixed_ts=${fixed_ts}, seed=${seed} ==="
